...
export function TaskDetailModal({ tasks, projectId, myRole }: TaskDetailModalProps) {
  ...
  return (
    ...
    >
      <div
        className="relative flex h-full w-full max-w-lg flex-col bg-white dark:bg-slate-900 shadow-2xl overflow-y-auto animate-slide-in"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-100 dark:border-slate-700/60 bg-white dark:bg-slate-900 px-6 py-4">
          <span className="text-xs font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">Task detail</span>
          <div className="flex items-center gap-2">
            {canDelete && (
              <button
                type="button"
                onClick={handleDelete}
                className="rounded-lg p-1.5 text-slate-400 dark:text-slate-500 hover:bg-red-50 dark:hover:bg-red-950/30 hover:text-red-500"
                title="Delete task"
              >
                <Trash2 size={15} />
              </button>
            ...
            <button
              type="button"
              onClick={closeTaskDetail}
              className="rounded-lg p-1.5 text-slate-400 dark:text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-700 dark:hover:text-slate-200"
            >
              <X size={16} />
            </button>
          </div>
        ...
        <div className="flex-1 px-6 py-5 space-y-6">
          ...
          <input
            ...
            readOnly={!canEdit}
            className={`w-full border-0 text-xl font-semibold text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-0 bg-transparent leading-snug
          ...
          <div className="flex flex-wrap gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              <span className="text-[10px] font-semibold uppercase tracking-widest text-slate-400 dark:text-slate-500 flex items-center gap-1">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input
                ...
                readOnly={!canEdit}
                className={`rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-2.5 py-1.5 text-xs font-medium text-slate-700 dark:text-slate-300
                  focus:border-indigo-300 focus:outline-none focus:ring-1 focus:ring-indigo-300
                  ${!canEdit ? 'opacity-60 cursor-default' : 'hover:border-indigo-300 dark:hover:border-indigo-700'}`}
              />
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            <textarea
              ...
              className={`w-full resize-none rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2.5 text-sm text-slate-700 dark:text-slate-300
                placeholder-slate-400 dark:placeholder-slate-600 focus:border-indigo-400 dark:focus:border-indigo-600 focus:outline-none focus:ring-2 focus:ring-indigo-400/20
        ...
        {/* Footer */}
        <div className="sticky bottom-0 border-t border-slate-100 dark:border-slate-700/60 bg-white dark:bg-slate-900 px-6 py-4 flex justify-end gap-2">
          <button
            type="button"
            onClick={closeTaskDetail}
